...
class CustomUserManager(BaseUserManager):
    ...
    """
    def create_user(self, mobile_number, email, password, **extra_fields):
    ...
    def create_superuser(self, mobile_number, email, password, **extra_fields):
        user = self.create_user(mobile_number, email, password, **extra_fields)
        user.is_admin=True
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(max_length=255, unique=True, blank=True, null=True, default='', help_text="User's Email")
